            ...
            <div class="page-content">
                <div class="page-bar">
                    <div class="page-title-breadcrumb">
                        <div class=" pull-left">
                            <div class="page-title">Item Category Sales Analysis</div>
                        </div>
                            ...
                            <div class="card-body ">
                                <div class="table-scrollable">
								   ...
								   <table  id="example"  class="table table-bordered table-hover" style="width:100%">   
                                    <div>
				                        <h3 class="text-center">Item Category Sales Analysis On <?php echo date('d-m-Y', strtotime($_POST['fromdate'])); ?><h3>				
		                            </div>                                 
                                     <thead> 
                                        <tr style="text-align: center;background-color:#c9c6c6;">
                                            ...
                                            <th>Item Category</th>    
                                            <th>To Day Sales</th> 
                                            <th>To Month Sales</th> 
                                            <th>Year To Date</th>                                                                                                                                                                             
                                        </tr>                                                                 
                                    </thead>
                                    <tbody>     
                                    <?php                                                      
                                    $query1="Exec Rep_ItemCategorySalesAnalysis ".$_POST['outlet'].",'".$_POST['fromdate']."','".date('Y-m-01', strtotime($_POST['fromdate']))."','".$_SESSION['MPOSCOMPANYID']."'";
                                    $result1=$this->db->query($query1);                                 
                                    $sales=0;$Ysales=0;$Msales=0;
                                    foreach ($result1->result_array() as $row1)	 
                                    {  
                                   echo ' <tr>                                           
                                            <td>'.$row1["Name"].'</td>    
                                            <td>'.$row1["itemcategory"].'</td>  
                                            <td style="text-align: right">'.number_format($row1["sales"],2).'</td>    
                                            <td style="text-align: right">'.number_format($row1["Msales"],2).'</td>    
                                            <td style="text-align: right">'.number_format($row1["Ysales"],2).'</td>                                                                                    
                                        </tr>';  
        ...
        $("#exporttable").click(function(e)
		{
          ...
          if(table && table.length)
		  {
            $(table).table2excel({
              ...
              filename: "Item Category Sales Analysis" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
              fileext: ".xls",
              exclude_img: true,
              exclude_links: true,
              exclude_inputs: true,
